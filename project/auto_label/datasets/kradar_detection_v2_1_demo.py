...
class KRadarDetection_v2_1_demo(Dataset):
    def __init__(self, cfg=None, split='all'):
        print('     > KRadarDetection_v2_1_demo dataset')
        if cfg == None:
        ...
        self.description_list = add_description_information(self.cfg)
    ...
    def load_dict_item(self, path_data, split):
        ...
        for seq, path_header in list_seqs_w_header:
            ...
            for label in list_labels:
                ...
                path_label_v1_1 = osp.join(self.cfg['path_data']['revised_label_v1_1'],str(seq),label)
                path_label_v2_0 = osp.join(self.cfg['path_data']['revised_label_v2_0'],str(seq),label)
                path_label_v2_1 = osp.join(self.cfg['path_data']['revised_label_v2_1'],str(seq),label)
                try: path_label_v_P = osp.join(self.cfg['path_data']['revised_label_v_P'],str(seq),label)
                except: path_label_v_P = None
                ...
                dict_item = dict(
                    meta = dict(
                        ...
                        label_v_P = path_label_v_P,
                        split = list_dict_split[int(seq)][label]
                    ),
                    desc = self.description_list[int(seq)]
                )
        ...
        if split == 'all':
            all_data_len = len(list_dict_item)
            print('     > all data are loaded : ', all_data_len)
            pass
        else:
            list_dict_item = list(filter(lambda item: item['meta']['split']==split, list_dict_item))
            data_len = len(list_dict_item)
            print('     > %s data are loaded : '%(split), data_len)

        data_len = len(list_dict_item)
        scene_filtered_len = len(list_dict_item)
        # V2_1 added : Filter SCENE frames from cfg.yaml file
        if self.all_cfg['IS_DATA_SCENE_FILTERING']:
            list_dict_item = [item for item in list_dict_item if item['desc'][2] in self.all_cfg['DATA_SCENE_FILTERING']]
            scene_filtered_len = len(list_dict_item)
            print('     > Data filtered based on scenes : ', ', '.join(self.all_cfg['DATA_SCENE_FILTERING']), '   // %d remaining data out of total %d'%(scene_filtered_len,data_len))
        ...
        if self.label.remove_0_obj:
            ...
            obj_data_len = len(list_dict_item)
            print('     > No object Data frame filtered : ',scene_filtered_len - obj_data_len, ' data filtered   // %d remaining data out of total %d'%(obj_data_len, data_len))
    ...
    def get_label(self, dict_item):
        ...
        if ver == 'v1_0':
            ...
            for line in lines[1:]:
                ...
                list_vals = line.rstrip('\n').split(',')
                if len(list_vals) != 12:
                    print('* split err in 1', path_label)
                    continue
                avail = (list_vals[1]).lstrip(' ').rstrip(' ')
                try:
                    idx_p = int(list_vals[2])
                    ...
                    idx_p = int(list_vals[2])
                    idx_b4 = float(list_vals[3])
                    #print('* split err in 2 ', path_label)
                    #continue
                cls_name = (list_vals[4]).lstrip(' ').rstrip(' ')
                x = float(list_vals[5]) + 2.53
                y = float(list_vals[6]) - 0.3
                z = float(list_vals[7]) - 0.7
                th = float(list_vals[8])*deg2rad
                l = 2*float(list_vals[9])
                w = 2*float(list_vals[10])
                h = 2*float(list_vals[11])
                list_tuple_objs.append((cls_name, 
                (x, y, z, th, l, w, h), (idx_p, idx_b4), avail))
        elif ver == 'v2_0':
            for line in lines[1:]:
                # print(line)
                list_vals = line.rstrip('\n').split(', ')
...
if __name__ == '__main__':
    kradar_detection = KRadarDetection_v2_1_demo(split='all')
