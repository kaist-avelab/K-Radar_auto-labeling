...
from utils.util_pipeline import update_dict_feat_not_inferenced_pvrcnn
...
from project.auto_label.datasets.kradar_detection_v2_1_demo import KRadarDetection_v2_1_demo

class PipelineDetection_v1_1():
    print('* PipelineDetection_v1_1')
    def __init__(self, path_cfg=None, mode='train', path_log=None):
        '''
        ...
        *   'test'  denotes mode for inference
        ...
        if self.mode == 'train':
            ...
            self.dataset_train = KRadarDetection_v2_1_demo(self.cfg, split='train')
            print('   - test dataset build')
            self.dataset_test = KRadarDetection_v2_1_demo(self.cfg, split='test')

        else: 
            print('   - test dataset build')
            self.dataset_test = KRadarDetection_v2_1_demo(self.cfg, split=mode)
            self.dataset_train = None

        if self.mode == 'all': self.mode = 'test'
    ...
    def set_logging(self, path_cfg, is_print_where=True):
        if self.mode == 'train': 
            ...
            if self.path_log == None: self.path_log = os.path.join(self.cfg.GENERAL.LOGGING.PATH_LOGGING, str_exp)
            else: print('* using previous path_log for test mode : ',self.path_log)
            if is_print_where:
                print(f'* Start logging in {str_exp}')
            if not (os.path.exists(self.path_log)):
                os.makedirs(self.path_log)
            else:
                print('* Exception error (Pipeline): same folder exists, continue saving')
                #exit()
            ...
            if self.is_save_model:
                if not os.path.exists(os.path.join(self.path_log, 'models')): os.makedirs(os.path.join(self.path_log, 'models'))
                if not os.path.exists(os.path.join(self.path_log, 'utils')): os.makedirs(os.path.join(self.path_log, 'utils'))
        ...
        for epoch in range(epoch_start, epoch_end):
            ...
            for idx_iter, dict_datum in enumerate(tqdm(data_loader_train)):
                try:
                    ...
                    traceback.print_exc()  # 여기서 실제 터미널에 출력되는 에러 메시지를 보여줍니다.
                    print('* Exception error (Pipeline): Error occured at training network step')
            try:
                if self.is_save_model:
                    # epoch: indexing from 0
                    path_dict_model = os.path.join(self.path_log, 'models', f'model_{epoch:02d}.pt')
                    path_dict_util = os.path.join(self.path_log, 'utils', f'util_{epoch:02d}.pt')
                ...
                traceback.print_exc()  # 여기서 실제 터미널에 출력되는 에러 메시지를 보여줍니다.
                print('* Exception error (Pipeline): Error occured at saving model step')
            ...
            try:
                ...
                traceback.print_exc()  # 여기서 실제 터미널에 출력되는 에러 메시지를 보여줍니다.
                print('* Exception error (Pipeline): Error occured at logging model step')
                ...
                if self.is_validate:
                    ...
                    if self.is_consider_subset:
                        if ((epoch + 1) % self.val_per_epoch_subset) == 0:
                            self.validate_kitti_conditional(epoch, list_conf_thr=self.list_val_conf_thr, is_subset=True)
                    if ((epoch + 1) % self.val_per_epoch_full) == 0:
                        self.validate_kitti_conditional(epoch, list_conf_thr=self.list_val_conf_thr)
            except Exception as e:
                traceback.print_exc()  # 여기서 실제 터미널에 출력되는 에러 메시지를 보여줍니다.
                print('* Exception error (Pipeline): Error occured at validation step')
    ...
    def load_dict_model(self, path_dict_model, is_strict=False):
        ...
        print('* dict_model loaded : ',path_dict_model)
        ...
        for idx_datum, dict_datum in enumerate(data_loader):
            ...
            for conf_thr in list_conf_thr:
                ...
                if is_feature_inferenced:
                    ...
                    #except: dict_out = self.network.list_modules[-1].post_processing(dict_out)
                else:
        ...
        for idx_datum, dict_datum in enumerate(data_loader):
            ...
            for conf_thr in list_conf_thr:
                ...
                try:
                    ...
                    traceback.print_exc()
                    print('* Exception error (Pipeline): Error occured at saving validation output')
            tqdm_bar.update(1)
        tqdm_bar.close()

        ### Validate per conf ###
        try:
            if epoch < self.cfg['OPTIMIZER']['MAX_EPOCH']-1: 
                all_condition_list = ['all']
                list_conf_thr = [0.3]
            else: all_condition_list = ['all'] + road_cond_list + time_cond_list + weather_cond_list
        except TypeError:
        ...
        for conf_thr in list_conf_thr:
            for condition in all_condition_list:
                try:
                    ...
                    val_ids = read_imageset_file(split_path)
                    dt_annos = kitti.get_label_annos(preds_dir, val_ids)
                    gt_annos = kitti.get_label_annos(labels_dir, val_ids)
                    list_metrics = []
                    list_results = []
                    ...
                    with open(os.path.join(path_dir, f'{conf_thr}', 'complete_results.txt'), 'w') as f:
                        print(os.path.join(path_dir, f'{conf_thr}', 'complete_results.txt'))
                        print(list_conf_thr)
                        print(conf_thr)
